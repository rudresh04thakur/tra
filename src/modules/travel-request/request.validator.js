...
module.exports = {
  ...
  validateRequest: (httpRequest) => {
    const schema = Joi.object({
      ...
      travelFrom: Joi.array().items(Joi.string()),
      travelTo: Joi.array().items(Joi.string()),
      workDestination: Joi.array().items(Joi.string()),
      travelDate: Joi.array().items(Joi.string()),
      numberOfDays: Joi.array().items(Joi.string()),
